<footer>
<div id="footer-con-container">
	<div id="wavebar-con-wrapper" class="dynamic-wavebar">
		<div class="scale-wavebar-hide visual-wavebar">
		</div>
		<div class="scale-wavebar-hide visual-wavebar delayed-fade" style="display:none;">
		</div>
	</div>
	<div id="footer-con-header">
		<div id="footer-con-outer">
			<div id="footer-con-inner">
				<div id="footer-ico-logo" class="darkmode-footer-logo">
				</div>
				<table id="footer-table-out">
				<tr>
					<td>
						<div id="footer-table-in">
							<div id="footer-con-bound">
								<div id="footer-tx1-bound" class="darkmode-bound-tx1">
									<span>Main</span>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="/blog">
									<span>Blog</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="/about">
									<span>About</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="/compatibility">
									<span>Compatibility</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="/quickstart">
									<span>Quickstart</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="/roadmap">
									<span>Roadmap</span>
									</a>
								</div>
							</div>
						</div>
					</td>
					<td>
						<div id="footer-table-in">
							<div id="footer-con-bound">
								<div id="footer-tx1-bound" class="darkmode-bound-tx1">
									<span>GitHub</span>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/RPCS3/rpcs3" target="_blank">
									<span>Repository</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/RPCS3/rpcs3/pulls" target="_blank">
									<span>Pull Requests</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/RPCS3/rpcs3/commits/master" target="_blank">
									<span>Commits</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/RPCS3/rpcs3/issues" target="_blank">
									<span>Issues</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/RPCS3/rpcs3/wiki" target="_blank">
									<span>Wiki</span>
									</a>
								</div>
							</div>
						</div>
					</td>
					<td>
						<div id="footer-table-in">
							<div id="footer-con-bound">
								<div id="footer-tx1-bound" class="darkmode-bound-tx1">
									<span>Social</span>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://discord.me/RPCS3" target="_blank">
									<span>Discord</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://forums.rpcs3.net" target="_blank">
									<span>Forums</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://www.youtube.com/channel/UCz3-0QxNr4S4gK0xaWy7exQ" target="_blank">
									<span>YouTube</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://twitter.com/rpcs3" target="_blank">
									<span>Twitter</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://www.reddit.com/r/rpcs3" target="_blank">
									<span>Reddit</span>
									</a>
								</div>
							</div>
						</div>
					</td>
					<td class="scale-menu-btn-remove">
						<div id="footer-table-in">
							<div id="footer-con-bound">
								<div id="footer-tx1-bound" class="darkmode-bound-tx1">
									<span>Webmasters</span>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/DAGINATSUKO" target="_blank">
									<span>daginatsuko</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/AniLeo" target="_blank">
									<span>anileo</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<a href="https://github.com/al0xf" target="_blank">
									<span>ssshadow</span>
									</a>
								</div>
								<div id="footer-tx2-bound" class="darkmode-bound-tx2">
									<span>shadow*</span>
								</div>
							</div>
						</div>
					</td>
				</tr>
				</table>
			</div>
		</div>
	</div>
	<div id='footer-con-credits'>
		<div id="footer-con-outer">
			<div id="footer-con-inner">
				<div id="footer-con-foot">
					<div id="footer-tx1-webmaster" class="darkmode-webmaster-tx1 scale-webmaster-txt-remove">
						<span><span>Website designed and maintained by </span><span id="footer-ico-webmaster" class="darkmode-webmaster-ico"></span><span><a style="pointer-events: all; padding-left: 22px;" href="https://daginatsuko.com" target="_blank" class="darkmode-webmaster-tx2">DAGINATSUKO</a></span></span>
					</div>
					<div id="footer-tx2-webmaster" class="darkmode-webmaster-tx2">
						<span>Founded by DH and Hykem © 2011 - 2018 RPCS3</span>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
</footer>